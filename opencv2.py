...
class Video:
    def __init__(self, path):
        ...
        if Path("static/data/"+self.filename+" - keyframes.npy").exists():
            print("load keyframes.npy")
            self.keyframes = np.load(
                "static/data/"+self.filename+" - keyframes.npy")
            print("keyframes.shape:", self.keyframes.shape)
        if Path("static/data/"+self.filename+" - diff_value.npy").exists():
            print("load diff_value.npy")
            self.diff_value = np.load(
                "static/data/"+self.filename+" - diff_value.npy")
    ...
    def ana_keyframes_process(self):
        ...
        np.save("static/data/"+self.filename+" - keyframes", self.keyframes)
        np.save("static/data/"+self.filename+" - diff_value", self.diff_value)
